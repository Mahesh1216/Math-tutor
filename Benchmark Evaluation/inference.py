# Modified inference.py to support Agentic backend (FastAPI)
import os
import re
...
from agentic_wrapper import call_agent, extract_answer  # ✅ Import wrapper
...
def extract_option_map(question_text: str) -> dict:
    """
    Extracts option letters and corresponding LaTeX values from the question.
    """
    matches = re.findall(r'\(([A-D])\)\s*\$([^$]+)\$', question_text)
    return {letter: expr.strip() for letter, expr in matches}
...
def get_response(question, model, model_nickname, mode, response_file, lock):
	...
	option_map = extract_option_map(question["question"]) if question["type"] == "MCQ" else None
	extract = extract_answer(answer, option_map=option_map)

	
	response_dict[f"{model_nickname}_{mode}_response"] = {
		"choices": [{"message": {"content": answer}, "extract": extract}]
	}
...
def main():
	...
	args.add_argument('--model', default='agentic')
	args.add_argument('--data', default='data/dataset.json')
	args.add_argument('--mode', default='CoT')
	...
	if os.path.exists(out_file):
				...
				with open(out_file, 'r') as infile:
					...
					for i, old_resp in enumerate(responses):
						if question['type'] in ['Numeric', 'Integer'] and args.mode == 'CoT+Exam':
							found = True
						if old_resp['description'] == question['description'] and old_resp['index'] == question['index']:
							found = all([old_resp.get(f"{model_nickname[args.model]}_{args.mode}_response", False) for model in [args.model]])
					if found:
		...
		if args.mode == 'CoT+Exam':
			rem_ques = [q for q in questions if q['type'] in ['MCQ', 'MCQ(multiple)']]
		else:
